...
namespace ArcSoftFace.Utils
{
    public class FaceUtil
    {
        ...
        /// <returns>保存人脸特征结构体指针列表</returns>
        public static List<IntPtr> ExtractFeature(IntPtr pEngine, ImageInfo imageInfo, ASF_MultiFaceInfo multiFaceInfo, out ASF_SingleFaceInfo singleFaceInfo)
        {
            List<IntPtr> featureList = new List<IntPtr>();
        ...
        public static List<IntPtr> ExtractFeature(IntPtr pEngine, Image image, out ASF_SingleFaceInfo singleFaceInfo)
        {
            image = ImageUtil.ScaleImage(image,image.Width,image.Height);
            ImageInfo imageInfo = ImageUtil.ReadBMP(image);
            ASF_MultiFaceInfo multiFaceInfo = DetectFace(pEngine, imageInfo);
            singleFaceInfo = new ASF_SingleFaceInfo();
            List<IntPtr> featureList = ExtractFeature(pEngine, imageInfo, multiFaceInfo, out singleFaceInfo);
            MemoryUtil.Free(imageInfo.imgData);
            return featureList;
        }
